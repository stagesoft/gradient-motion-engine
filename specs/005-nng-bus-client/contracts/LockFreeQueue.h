...
bool LockFreeQueue<T, N>::push(T&& item) noexcept {
    ...
    bool dropped = false;
    // Full? Advance tail (drop oldest) so the new item can land.
    std::size_t tail = tail_.load(std::memory_order_acquire);
    if (next == tail) {
        const std::size_t new_tail = (tail + 1) % N;
        // Single CAS is enough: the consumer only ever advances tail forward,
        // so if the consumer popped in between, our CAS simply fails and we
        // re-observe a now-non-full queue and skip the drop.
